...
/// <summary>
/// Linux OpenGL rasterizer. Commands are rendered by Cairo into BGRA pixels and then
/// presented through a real GLX/OpenGL context.
/// </summary>
[SupportedOSPlatform("linux")]
public sealed class OpenGLRasterizer : IRasterizer
{
    private readonly IRasterizer _bitmapBackend = new CairoRasterizer();
    private readonly IRasterizer _textBackend = new CairoRasterizer();
    ...
    private int _textureFilter = (int)Glx.GL_LINEAR;
    ...
    public void Initialize(NativeWindowHandle window, int width, int height)
    {
        if (window.Kind != "X11")
        {
            throw new ArgumentException($"OpenGlRasterizer requires X11, got '{window.Kind}'.");
        }
        ...
        {
            throw new PlatformNotSupportedException("Failed to initialize Linux OpenGL (GLX).");
        }
    ...
    public void InitializeBitmap(int width, int height)
    {
        ...
        if (_display == 0)
        {
            throw new PlatformNotSupportedException("Failed to open X11 display for OpenGL bitmap mode.");
        }

        _ownsDisplay = true;
        int screen = X11.XDefaultScreen(_display);
        nint root = X11.XRootWindow(_display, screen);
        _window = X11.XCreateSimpleWindow(_display, root, 0, 0, (uint)Math.Max(1, width), (uint)Math.Max(1, height), 0, 0, 0);
        if (_window == 0)
        {
            throw new PlatformNotSupportedException("Failed to create X11 window for OpenGL bitmap mode.");
        }
    ...
    public void Resize(int width, int height)
    {
        ...
        if (_offscreenGl && _display != 0 && _window != 0)
        {
            X11.XResizeWindow(_display, _window, (uint)Math.Max(1, width), (uint)Math.Max(1, height));
            X11.XFlush(_display);
        }

        if (_glContext != 0 && _display != 0 && _window != 0)
        {
            Glx.glXMakeCurrent(_display, _window, _glContext);
            Glx.glViewport(0, 0, width, height);
        }
    }
    ...
    public void EndFrame(ReadOnlySpan<DrawCommand> commands)
    {
        ...
        if (!Glx.glXMakeCurrent(_display, _window, _glContext))
        {
            return;
        }
        ...
        PixelOffsetMode pixelOffset, CompositingQuality compositing)
    {
        ...
        {
            InterpolationMode.NearestNeighbor => (int)Glx.GL_NEAREST,
            _ => (int)Glx.GL_LINEAR,
        };
    ...
    public void Dispose()
    {
        if (_glContext != 0 && _display != 0 && _window != 0)
        {
            ...
            {
                uint texture = tex;
                Glx.glDeleteTextures(1, ref texture);
            }
        ...
        if (_glContext != 0 && _display != 0)
        {
            Glx.glXMakeCurrent(_display, 0, 0);
            Glx.glXDestroyContext(_display, _glContext);
            _glContext = 0;
        }

        if (_ownsWindow && _display != 0 && _window != 0)
        {
    ...
    private bool TryInitializeOpenGlWindow()
    {
        try
        {
            ...
            nint visualInfo = Glx.glXChooseVisual(_display, screen, attributes);
            if (visualInfo == 0)
            {
                return false;
            }
            ...
            try
            {
                _glContext = Glx.glXCreateContext(_display, visualInfo, 0, true);
                if (_glContext == 0)
                {
                    return false;
                }
                ...
                Glx.glViewport(0, 0, _width, _height);
                return true;
            }
            finally
            {
    ...
    {
        Glx.glViewport(0, 0, _width, _height);
        Glx.glClearColor(0f, 0f, 0f, 1f);
        Glx.glClear(Glx.GL_COLOR_BUFFER_BIT);

        Glx.glMatrixMode(Glx.GL_PROJECTION);
        Glx.glLoadIdentity();
        Glx.glOrtho(0, _width, _height, 0, -1, 1);
        Glx.glMatrixMode(Glx.GL_MODELVIEW);
        Glx.glLoadIdentity();

        Glx.glEnable(Glx.GL_BLEND);
        Glx.glBlendFunc(Glx.GL_SRC_ALPHA, Glx.GL_ONE_MINUS_SRC_ALPHA);
        if (_lineSmoothing)
        {
            Glx.glEnable(Glx.GL_LINE_SMOOTH);
        }
        else
        {
            Glx.glDisable(Glx.GL_LINE_SMOOTH);
        }
        if (_dither)
        {
            Glx.glEnable(Glx.GL_DITHER);
        }
        else
        {
            Glx.glDisable(Glx.GL_DITHER);
        }
        Glx.glDisable(Glx.GL_SCISSOR_TEST);
    }
        ...
        for (int i = 0; i < commands.Length; i++)
        {
            ...
            switch (cmd.Kind)
            {
                case DrawCommandKind.Clear:
                    Glx.glClearColor(cmd.Color.R, cmd.Color.G, cmd.Color.B, cmd.Color.A);
                    Glx.glClear(Glx.GL_COLOR_BUFFER_BIT);
                    break;
    ...
    private void CapturePixels()
    {
        ...
        unsafe
        {
            ...
            {
                Glx.glReadPixels(0, 0, _width, _height, Glx.GL_BGRA, Glx.GL_UNSIGNED_BYTE, (nint)ptr);
            }
        }
    ...
    {
        SetColor(cmd.Color);
        Glx.glLineWidth(MathF.Max(1f, cmd.StrokeWidth));
        PointF p0 = TransformPoint(cmd.P1);
        PointF p1 = TransformPoint(cmd.P2);

        if (cmd.DashStyle == DashStyle.Solid)
        {
            Glx.glBegin(Glx.GL_LINES);
            Glx.glVertex2f(p0.X, p0.Y);
            Glx.glVertex2f(p1.X, p1.Y);
            Glx.glEnd();
            return;
        }
    ...
    private void EnsureTextureUploaded(ref uint texture, int texWidth, int texHeight, byte[] bgraPixels)
    {
        if (texture == 0)
        {
            Glx.glGenTextures(1, out texture);
        }

        Glx.glBindTexture(Glx.GL_TEXTURE_2D, texture);
        Glx.glTexParameteri(Glx.GL_TEXTURE_2D, Glx.GL_TEXTURE_MIN_FILTER, _textureFilter);
        Glx.glTexParameteri(Glx.GL_TEXTURE_2D, Glx.GL_TEXTURE_MAG_FILTER, _textureFilter);
        Glx.glTexParameteri(Glx.GL_TEXTURE_2D, Glx.GL_TEXTURE_WRAP_S, (int)Glx.GL_CLAMP_TO_EDGE);
        Glx.glTexParameteri(Glx.GL_TEXTURE_2D, Glx.GL_TEXTURE_WRAP_T, (int)Glx.GL_CLAMP_TO_EDGE);
    ...
    private void DrawTextureCached(uint texture, int texWidth, int texHeight, PointF origin, float width, float height, float opacity)
    {
        ...
        Glx.glEnable(Glx.GL_TEXTURE_2D);
        Glx.glBindTexture(Glx.GL_TEXTURE_2D, texture);
        ...
        Glx.glColor4f(1f, 1f, 1f, Math.Clamp(opacity, 0f, 1f));
        Glx.glBegin(Glx.GL_QUADS);
        Glx.glTexCoord2f(0f, 0f); Glx.glVertex2f(p0.X, p0.Y);
        Glx.glTexCoord2f(u1, 0f); Glx.glVertex2f(p1.X, p1.Y);
        Glx.glTexCoord2f(u1, v1); Glx.glVertex2f(p2.X, p2.Y);
        Glx.glTexCoord2f(0f, v1); Glx.glVertex2f(p3.X, p3.Y);
        Glx.glEnd();
        Glx.glDisable(Glx.GL_TEXTURE_2D);
    }
    ...
    private void DrawTexture(ref uint texture, int texWidth, int texHeight, byte[] bgraPixels, RectF dest, RectF? src, float opacity)
    {
        ...
        Glx.glTexCoord2f(u0, v0);
        Glx.glVertex2f(p0.X, p0.Y);

        Glx.glTexCoord2f(u1, v0);
        Glx.glVertex2f(p1.X, p1.Y);

        Glx.glTexCoord2f(u1, v1);
        Glx.glVertex2f(p2.X, p2.Y);

        Glx.glTexCoord2f(u0, v1);
        Glx.glVertex2f(p3.X, p3.Y);
    ...
    private void DrawDashedLine(PointF p0, PointF p1, DashStyle style, float strokeWidth)
    {
        ...
        while (t < length)
        {
            ...
            {
                Glx.glBegin(Glx.GL_LINES);
                Glx.glVertex2f(p0.X + (nx * t0), p0.Y + (ny * t0));
                Glx.glVertex2f(p0.X + (nx * t1), p0.Y + (ny * t1));
                Glx.glEnd();
            }
    ...
    private void ApplyClip()
    {
        ...
        Glx.glEnable(Glx.GL_SCISSOR_TEST);
        Glx.glScissor(left, glY, w, h);
    }
    ...
    {
        Glx.glColor4f(color.R, color.G, color.B, color.A);
    }
